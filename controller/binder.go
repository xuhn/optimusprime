...
func bindStruct(params *Params, name string, typ reflect.Type) reflect.Value {
	...
	if params.JSON != nil {
		...
		if err := json.Unmarshal(params.JSON, resultPointer.Interface()); err != nil {
			log.WARNF("W: bindStruct: Unable to unmarshal request:", name, err)
		}
		return result
	}
	...
	for key := range params.Values {
		...
		if _, ok := fieldValues[fieldName]; !ok {
			...
			if !fieldValue.IsValid() {
				log.WARNF("W: bindStruct: Field not found:", fieldName)
				continue
			}
			if !fieldValue.CanSet() {
				log.WARNF("W: bindStruct: Field not settable:", fieldName)
				continue
			}
...
func getMultipartFile(params *Params, name string) multipart.File {
	for _, fileHeader := range params.Files[name] {
		...
		log.WARNF("Failed to open uploaded file", name, ":", err)
	}
	return nil
}
...
func bindFile(params *Params, name string, typ reflect.Type) reflect.Value {
	...
	if err != nil {
		log.WARNF("Failed to create a temp file to store upload:", err)
		return reflect.Zero(typ)
	}
	...
	if err != nil {
		log.WARNF("Failed to copy upload to temp file:", err)
		return reflect.Zero(typ)
	}

	_, err = tmpFile.Seek(0, 0)
	if err != nil {
		log.WARNF("Failed to seek to beginning of temp file:", err)
		return reflect.Zero(typ)
	}
...
func bindByteArray(params *Params, name string, typ reflect.Type) reflect.Value {
	if reader := getMultipartFile(params, name); reader != nil {
		...
		log.WARNF("Error reading uploaded file contents:", err)
	}
	return reflect.Zero(typ)
}
...
func bindMap(params *Params, name string, typ reflect.Type) reflect.Value {
	...
	if params.JSON != nil {
		...
		if err := json.Unmarshal(params.JSON, resultPtr.Interface()); err != nil {
			log.WARNF("W: bindMap: Unable to unmarshal request:", name, err)
		}
		return result
	}
...
func binderForType(typ reflect.Type) (Binder, bool) {
	...
	if !ok {
		...
		if !ok {
			log.WARNF("revel/binder: no binder for type:", typ)
			return Binder{}, false
		}
	}
	return binder, true
...
func init() {
	...
	OnAppStart(func() {
		DateTimeFormat = common.StringDefault("format.datetime", DefaultDateTimeFormat)
		DateFormat = common.StringDefault("format.date", DefaultDateFormat)
