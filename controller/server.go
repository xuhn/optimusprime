	...
	if upgrade == "websocket" || upgrade == "Websocket" {
		websocket.Handler(func(ws *websocket.Conn) {
			...
			if err := ws.SetDeadline(time.Now().Add(time.Hour * 24)); err != nil {
				log.ERRORF("SetDeadLine failed:", err)
			}
			r.Method = "WS"
...
func InitServer() http.HandlerFunc {
	...
	/*
		...
		if xfcommon.BoolDefault("watch", true) {
		...
		if MainWatcher != nil && xfcommon.BoolDefault("watch.templates", true) {
...
func Run(port int) {
	...
	Server = &http.Server{
		...
		ReadTimeout:  time.Duration(common.IntDefault("http.timeout.read", 0)) * time.Second,
		WriteTimeout: time.Duration(common.IntDefault("http.timeout.write", 0)) * time.Second,
	}
	...
	if HTTPSsl {
		if network != "tcp" {
			...
			log.FATALF("SSL is only supported for TCP sockets. Specify a port to listen on.")
		}
		...
		log.FATALF("Failed to serve:", Server.Serve(listener))
	}
}
